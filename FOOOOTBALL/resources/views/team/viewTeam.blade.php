<div id="dropdown-content">
    ...
    <div class="flex flex-col items-center relative border-2 border-gray-400 rounded-lg p-8 mx-8 max-h-72 bottom-4">
        ...
        <div class="mb-4">
            ...
            <strong class="text-gray-400">Name :</strong> {{ $team->name }}
            </span>
        </div>
        <div class="mb-4">
            <span class="w-full py-2 px-3 text-gray-400 leading-tight focus:outline-none focus:shadow-outline">
            <strong class="text-gray-400">AKA :</strong> {{ $team->AKA }}
            </span>
        </div>
        <div class="mb-4">
            <span class="w-full py-2 px-3 text-gray-400 leading-tight focus:outline-none focus:shadow-outline">
            <strong class="text-gray-400">Type :</strong> {{ $team->type }}
            </span>
        </div>
        <div class="mb-12">
            <span class="w-full py-2 px-3 text-gray-400 leading-tight focus:outline-none focus:shadow-outline">
            <strong class="text-gray-400">Location :</strong> {{ $team->location }}
            </span>
        </div>
        <div class="flex justify-around py-3 z-10">
            <div class="relative mx-4">
                <button type="button" data-name="Update" data-id="{{ $team->id }}" class="viewTeamUpdate w-4 h-4 bg-blue-400 rounded-full transform hover:scale-150 
                                                                        transition-transform duration-200 outline-none"></button>
            </div>
            <div class="relative mx-4">
                <button type="button" data-name="Delete" data-id="{{ $team->id }}" class="viewTeamDelete w-4 h-4 bg-red-400 rounded-full transform hover:scale-150 
                                                                        transition-transform duration-200 outline-none"></button>
            </div>
        </div>
